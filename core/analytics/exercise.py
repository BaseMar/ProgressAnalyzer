...
class ExerciseAnalytics:
    ...
    def compute_session_summary(self, df_ex: pd.DataFrame) -> pd.DataFrame:
        df = df_ex.copy()
        df["Est1RM"] = compute_est_1rm(self.df["Weight"], self.df["Repetitions"])
        df["TotalVolume"] = df["Weight"] * df["Repetitions"]

        session_summary = (df.groupby("SessionDate", as_index=False).agg(Est1RM=("Est1RM", "max"), TotalVolume=("TotalVolume", "sum"), Weight=("Weight", "mean")).sort_values("SessionDate").reset_index(drop=True))
    ...
    def compute_history_table(self, df_ex: pd.DataFrame) -> pd.DataFrame:
        ...
        df_summary = (df.groupby("SessionDate", as_index=False).agg(SeriesCount=("SetNumber", "count"),TotalReps=("Repetitions", "sum"), AvgWeight=("Weight", "mean"), Volume=("Volume", "sum")).sort_values("SessionDate").reset_index(drop=True))
